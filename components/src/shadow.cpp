...
namespace Element
{
    void BackdropEffect::setShadow(QWidget* widget, ShadowType type)
    {
        if (!widget) return;

        QGraphicsDropShadowEffect* shadow = new QGraphicsDropShadowEffect(widget);

        if (type == ShadowType::Basic)
        {
            shadow->setBlurRadius(32);
            shadow->setOffset(0, 12);
            shadow->setColor(QColor(0, 0, 0, 0.04 * 255));
        }
        else if (type == ShadowType::Light)
        {
            shadow->setBlurRadius(12);
            shadow->setOffset(0, 0);
            shadow->setColor(QColor(0, 0, 0, 0.12 * 255));
        }
        else if (type == ShadowType::Lighter)
        {
            shadow->setBlurRadius(6);
            shadow->setOffset(0, 0);
            shadow->setColor(QColor(0, 0, 0, 0.12 * 255));
        }
        else if (type == ShadowType::Dark)
        {
    ...
    Mask* BackdropEffect::setBgMask(QWidget* widget)
    {
