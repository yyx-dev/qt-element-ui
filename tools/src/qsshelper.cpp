...
namespace QTElementUI
{
    ...
    // 向指定组添加属性
    void QSSHelper::setProperty(const QString &groupName, const QString &key, const QString &value)
    {
        // 如果组不存在，先创建空组
    ...
    // 从指定组移除属性
    void QSSHelper::removeProperty(const QString &groupName, const QString &property)
    {
        if (_styleGroups.contains(groupName))
        {
            QStringList &properties = _styleGroups[groupName];
